...
export class BooksService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.googleBooksAPIKey = "AIzaSyC5IAKdiiU8zMoaIpC1MWIrA6qxAxIIugo";
  }
  ...
  getBooks(libraryId: number): Observable<Book[]> {
    ...
    return this.http
      .get<LibraryBook[]>(url)
      .pipe(map((items: LibraryBook[]) => items.map((item) => item.book)));
  }
  ...
  getTotalNumberOfCopiesInLibrary(
    ...
    return this.http.get<LibraryBook[]>(url, {}).pipe(
      map((libbooks) => libbooks.filter((b) => b.book.bookId === bookId)),
      map((arr) => arr[0].totalPurchasedByLibrary)
    );
  }

  getLibrary(libraryId: number): Observable<LibraryBook[]> {
    const url = `${this.apiUrl}${libraryId}/books`;
